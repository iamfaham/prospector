...
def _populated_store(tmp_path):
    ...
    mid = store.insert_match(Match(role_variant_id=rv_id, company_id=cid, score=8, reasoning="strong match"))
...
def test_drafts_application_blurb_when_job_present(tmp_path):
    ...
    mid2 = store.insert_match(Match(role_variant_id=rv_id, company_id=cid, job_id=jid, score=9, reasoning="perfect"))
    store.update_match_status(mid2, MatchStatus.ACCEPTED)
    llm = MagicMock(spec=LLMClient)
    llm.is_over_budget.return_value = False
